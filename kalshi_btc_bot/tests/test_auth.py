...
def test_sign_returns_base64(auth: KalshiAuth) -> None:
    ...
    sig = auth.sign(1700000000, "GET", "/trade-api/v2/markets")
...
def test_sign_deterministic_for_same_input(auth: KalshiAuth) -> None:
    ...
    sig1 = auth.sign(1700000000, "GET", "/trade-api/v2/markets")
    sig2 = auth.sign(1700000000, "GET", "/trade-api/v2/markets")
    # PSS signatures are non-deterministic, so they may differ
...
def test_sign_different_for_different_method(auth: KalshiAuth) -> None:
    ...
    sig_get = auth.sign(1700000000, "GET", "/trade-api/v2/markets")
    sig_post = auth.sign(1700000000, "POST", "/trade-api/v2/markets")
...
def test_headers_timestamp_is_numeric(auth: KalshiAuth) -> None:
    headers = auth.headers("GET", "/trade-api/v2/markets")
    ts = headers["KALSHI-ACCESS-TIMESTAMP"]
    assert ts.isdigit()
    assert int(ts) > 1_000_000_000  # reasonable epoch
